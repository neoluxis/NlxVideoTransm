...
struct Frame
{
    ...
    Frame() = default;
    explicit Frame(std::vector<uchar> &&d) : data(std::move(d)) {}
};
...
private:
    ...
    const size_t max_size_ = 5; // Reduced to minimize latency
...
void image_capture_thread(cv::VideoCapture &cap, FrameQueue &frame_queue,
                          std::atomic<bool> &snapshot_signal,
                          int width, int height, int snapw, int snaph)
{
    std::vector<int> encode_params = {cv::IMWRITE_JPEG_QUALITY, 50}; // Reduced quality
    ...
    try
    {
        while (running)
        {
            ...
            if (snapshot_signal)
            {
                ...
                std::cout << "[" << get_timestamp() << "] cap.set (snapshot resolution) time: " << duration << " us\n";
                ...
                std::cout << "[" << get_timestamp() << "] cap.read (snapshot) time: " << duration << " us\n";
                ...
                duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
                std::cout << "[" << get_timestamp() << "] cap.set (revert resolution) time: " << duration << " us\n";
            ...
            else
            {
                ...
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
                std::cout << "[" << get_timestamp() << "] cap.read (normal) time: " << duration << " us\n";
            }
            ...
            auto end = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            std::cout << "[" << get_timestamp() << "] imencode time: " << duration << " us\n";

            // Push to queue
            frame_queue.push(Frame(std::move(buffer)));
            buffer = std::vector<uchar>(100000); // Reallocate for next frame

            auto end_total = std::chrono::high_resolution_clock::now();
            duration = std::chrono::duration_cast<std::chrono::microseconds>(end_total - start_total).count();
            std::cout << "[" << get_timestamp() << "] Total capture+encode time: " << duration << " us\n";
        }
    }
    catch (const std::exception &e)
    {
        std::cerr << "[" << get_timestamp() << "] Image capture thread exception: " << e.what() << "\n";
    }
}

// Handle single client
void handle_client(int client_socket, FrameQueue &frame_queue)
{
    // Set larger send buffer
    int sndbuf = 1024 * 1024; // 1MB
    setsockopt(client_socket, SOL_SOCKET, SO_SNDBUF, &sndbuf, sizeof(sndbuf));

    // Optionally disable TCP_NODELAY
    // int flag = 0;
    ...
    try
    {
        while (running && client_socket == current_client)
        {
            ...
            {
                break; // Queue empty and not running
            }
...
int init_serial(const std::string &serial, int baudrate)
{
    ...
    // Disable software flow control and input processing
...
int main(int argc, char *argv[])
{
    ...
    int fwidth = 1280, fheight = 480, snaph = 1280, snapw = 480; // Updated to match 1280x480
    int fps = 30;
    ...
    std::thread capture_thread(image_capture_thread, std::ref(cap), std::ref(frame_queue),
                               std::ref(snapshot_signal), fwidth, fheight, snapw, snaph);
    ...
    while (running)
    {
        ...
        {
            if (running)
                std::cerr << "[" << get_timestamp() << "] Poll error\n";
            continue;
        }
        if (ret == 0)
            continue;
        ...
        if (pfd.revents & POLLIN)
        {
            ...
            {
                if (running)
                    std::cerr << "[" << get_timestamp() << "] Accept failed\n";
                continue;
            }
            ...
            // Start client send thread
            std::thread client_thread(handle_client, client_fd, std::ref(frame_queue));
    ...
    running = false;
    frame_queue.push(Frame()); // Unblock queue
